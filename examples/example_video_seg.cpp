// example_video_seg — RF-DETR instance segmentation on a video file.
//
// Usage:
//   ./example_video_seg <engine> <video> [threshold]
//
// Example:
//   ./example_video_seg rf-detr-seg-nano-fp16.engine video.mp4 0.5
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const std::string out_path = "out/video_seg_" + utils::timestamp() + ".mp4";
        ...
        // Warm-up.
        cv::Mat tmp;
        if (cap.read(tmp)) segmenter.segment(tmp, threshold);
        ...
        while (cap.read(frame)) {
            ...
            rfdetr::Detections results = segmenter.segment(frame, threshold);
            const auto t1 = std::chrono::steady_clock::now();
            const double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
            total_ms += ms;
            ++frame_idx;

            rfdetr::draw_segmentations(frame, results);
        ...
        if (frame_idx > 0)
            utils::print_metrics("video_seg", total_ms / frame_idx, 1000.0 / (total_ms / frame_idx));
