...
// Public instance-segmentation API. Hides all TRT/CUDA headers behind PIMPL.
...
class RFDetrSegmenter {
   public:
    ...
    // Run segmentation on a single image. Returns detections with `.mask` populated.
    [[nodiscard]] Detections segment(const cv::Mat& image, float threshold = -1.0f);

    // Batch segmentation. Engine must be built with max_batch >= images.size().
    [[nodiscard]] std::vector<Detections> segment_batch(const std::vector<cv::Mat>& images,
    ...
    [[nodiscard]] int                mask_h()      const noexcept;
    [[nodiscard]] int                mask_w()      const noexcept;
