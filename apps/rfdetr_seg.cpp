// rfdetr_seg — single-image instance segmentation via the RFDetrSegmenter public API.
//
// usage:
//   rfdetr_seg --engine ENGINE --image IMAGE
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        rfdetr::RFDetrSegmenter segmenter(args.engine, opts);

        std::printf("engine:  %s\n", args.engine.c_str());
        std::printf("variant: %s  input=%dx%d  queries=%d  classes=%d  mask=%dx%d\n",
                    segmenter.variant().c_str(),
                    segmenter.input_w(), segmenter.input_h(),
                    segmenter.num_queries(), segmenter.num_classes(),
        ...
        segmenter.segment(image, args.threshold);

        rfdetr::Detections results;
        const auto t0 = std::chrono::steady_clock::now();
        for (int k = 0; k < args.iters; ++k) {
            results = segmenter.segment(image, args.threshold);
        }
        const auto t1  = std::chrono::steady_clock::now();
        const double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();

        const auto& t = segmenter.last_timings();
